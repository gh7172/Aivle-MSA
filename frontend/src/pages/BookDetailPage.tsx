...
const BookDetailPage: React.FC = () => {
  ...
  const { points: userPoints, isSubscribed } = useSelector((state: RootState) => state.user);
  ...
  // '바로 읽기' 버튼 클릭 시 실행될 함수
  const handleReadNow = () => {
    if (!book) return;
    // 실제로는 뷰어(Viewer) 페이지로 이동하는 로직이 필요합니다.
    alert(`'${book.title}' 책을 열람합니다.`);
    // 예시: navigate(`/viewer/${book.id}`);
  };
      ...
      <div className={styles.bookDetail}>
        ...
        <div className={styles.info}>
          ...
          <div className={styles.subscribeBox}>
            {isSubscribed ? (
              // --- 구독자일 경우 ---
              <button onClick={handleReadNow} className={styles.readNowButton}>
                바로 읽기
              </button>
            ) : (
              // --- 비구독자일 경우 ---
