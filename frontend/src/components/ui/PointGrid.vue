    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="chargePoints" class="contrast-primary-text" small color="primary" :disabled="!hasRole('user')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>포인트 충전
                </v-btn>
            </div>
            <GetPoint @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></GetPoint>
...
export default {
    ...
    methods:{
        async chargePoints(){
            try{
                var path = "chargePoints".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','chargePoints 성공적으로 처리되었습니다.')
