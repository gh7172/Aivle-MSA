    ...
    <v-card outlined style="border-radius: 0; margin-top: 10px; padding-bottom: 10px; background-color: #F4F5FA;">
        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                ...
                <v-col style="max-width:140px;">
                    <Date class="attributes-list" label="SubscriptionExpiryDate" v-model="value.parameters.subscriptionExpiryDate" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        ...
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'subscribes/search/getSubscribe',
                parameters: {}
            },
        }),
        created() {
